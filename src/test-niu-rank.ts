...
const VUE_FILENAME = '/niu-rank.vue';
...
async function generateDefaultProps(filename: string): Promise<any> {
  try {
    // 为 niu-rank.vue 组件准备数据
    // 生成20条原始数据
    const rawData = Array.from({ length: 20 }, (_, i) => {
      // 生成不同的用户数据
      const adjectives = ['超级', '无敌', '霹雳', '神奇', '威猛', '霸气', '炫酷', '厉害'];
      const nouns = ['牛仔', '公牛', '犀牛', '蜗牛', '海牛', '牦牛', '奶牛', '蛮牛', '金牛', '火牛'];
      const adjective = adjectives[Math.floor(Math.random() * adjectives.length)];
      const noun = nouns[Math.floor(Math.random() * nouns.length)];
      
      return {
        userId: 100000000 + i,
        nickname: `${adjective}${noun}${i + 1}`,
        item: { 
          length: Math.floor(Math.random() * 100) + 50, // 50-150之间的随机数
          injectedCount: Math.floor(Math.random() * 25), // 0-24之间的随机数
          ejaculateCount: Math.floor(Math.random() * 10),
          charm: Math.floor(Math.random() * 40) + 60 // 60-100之间的随机数
        }
      };
    });
    ...
    // 数据处理
    // 数据处理
    const sortedRawData = [...rawData].sort((a, b) => b.item.length - a.item.length);
    const maxLength = sortedRawData[0]?.item.length || 1;
    const sortedData: any = sortedRawData
      .slice(0, 20)
      .map((user, index) => {
        const rank = index + 1;
        const percent = ((user.item.length / maxLength) * 100).toFixed(1);
        return {
          ...user,
          rank,
